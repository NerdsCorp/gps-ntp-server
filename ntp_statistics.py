...
class NTPClient:
    ...
    def query_server(self, server, port=123):
        ...
        try:
            ...
            root_delay = unpacked[4] / 65536.0  # Convert to seconds
            ...
            current_ntp = (datetime.now(timezone.utc) - NTP_EPOCH).total_seconds()
            offset = ((recv_timestamp - current_ntp) + (trans_timestamp - current_ntp)) / 2
            
            # Decode reference ID
            if stratum == 0 or stratum == 1:
                # Reference identifier is ASCII string
                ref_id_str = struct.pack('!I', ref_id).decode('ascii', errors='ignore').strip('\x00')
            else:
                # Reference identifier is IP address
            ...
            return {
                ...
                'rtt': rtt,
                'latency': rtt / 2,  # Approximate one-way latency
                'poll_interval': 2 ** poll,
                'li': (li_vn_mode >> 6) & 0x3,  # Leap indicator
                'version': (li_vn_mode >> 3) & 0x7,  # NTP version
...
class NTPMonitor:
    ...
    def __init__(self, servers=None, history_size=3600):
        ...
        self.metrics = defaultdict(lambda: {
            ...
            'failed_queries': 0,
            'total_offset': 0,
            'offset_squares': 0,  # For standard deviation
            'last_success': None,
            'last_failure': None,
            'availability': 100.0,
...
def init_ntp_monitor(servers=None):
    ...
    default_servers = [
        ...
        # Add local GPS server
        {'address': '127.0.0.1', 'port': 123, 'name': 'Local GPS (This Server)'},
    ]
    ...
    # Start monitoring
    ntp_monitor.start(interval=30)  # Query every 30 seconds
...
<body>
    ...
    <script>
        ...
        function updateCharts(data) {
            ...
            // Update offset chart with color coding
            ...
            // Update historical chart if we have history data
            if (data.history) {
...
def api_ntp_stats():
    ...
    # Add limited history for charts (last 20 points per server)
    for server_config in ntp_monitor.servers[:5]:  # Limit to first 5 servers for performance
        server = server_config['address']
        history = ntp_monitor.get_server_history(server, 600)  # Last 10 minutes
        if history:
            # Sample every nth point to limit data
